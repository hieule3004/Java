...
public class Graph implements GraphInterface{
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
		...
		QueueInterface<GraphNode> tmpQueue = new Queue<>();
		QueueInterface<GraphNode> resultQueue = new Queue<>();

		setIncomingDegree();
		tmpQueue.enqueue(start);

		while (!tmpQueue.isEmpty()) {
			GraphNode node = tmpQueue.dequeue();
			resultQueue.enqueue(node);
			for (GraphEdge edge : node.getOutgoingEdges()) {
				GraphNode child = edge.getEndPoint();
				child.setDegree(child.getDegree() - 1);
				if (child.getDegree() == 0) {
					tmpQueue.enqueue(child);
				}
			}
		}
		return resultQueue;
	...
	private void computeEarliestCompletionTime( ) {
		...
		while (!sortedNodes.isEmpty()) {
			...
			for (GraphEdge edge : node.getIncomingEdges()) {
				time = Math.max(time, edge.getStartPoint().EarliestCompletionTime() + edge.getTaskDuration());
			}
			node.setEarliestCompletionTime(time);
		}
